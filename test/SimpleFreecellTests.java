...
/**
 * Class that tests behavior of SimpleFreecellModel, without exceptions as those
 * are tested in TestGetterExceptions.java and TestSFMExceptions.java.
 */
public class SimpleFreecellTests {
  private SimpleFreecellModel simpleFreecs1;
  private SimpleFreecellModel unstartedGame;
  private SimpleFreecellModel easiestGame;
  private SimpleFreecellModel midGame;

  // initializing fields
  @Before
  public void init() {
    simpleFreecs1 = new SimpleFreecellModel(new Random(67));
    ...
    easiestGame = new SimpleFreecellModel(new Random(7));
    easiestGame.startGame(easiestDeckToWin(), 4, 4, false);
    midGame = new SimpleFreecellModel(new Random(94));
  ...
  public void testGetDeck() {
    ...
    assertEquals(simpleFreecs1.getDeck(), deckCopy);
    assertEquals(simpleFreecs1.getDeck().size(), 52);
  }
  ...
  public void testRefresh() {
    ...
    String c2PreRefresh = "";
    for (int i = 0; i < simpleFreecs1.getNumCardsInCascadePile(1); i++) {
      c2PreRefresh += simpleFreecs1.getCascadeCardAt(1, i).toString() + " ";
    }
    assertEquals(c2PreRefresh, "2♣ 7♣ Q♣ 4♦ 9♦ A♥ 6♥ J♥ 3♠ 8♠ K♠ ");

    // restarting with.startGame()
    simpleFreecs1.startGame(simpleFreecs1.getDeck(), 7, 5, true);
    // testing the same traits post refresh
    assertEquals(simpleFreecs1.getNumCardsInCascadePile(0), 8);
    assertEquals(simpleFreecs1.getNumOpenPiles(), 5);
    String c2PostRefresh = "";
    for (int i = 0; i < simpleFreecs1.getNumCardsInCascadePile(1); i++) {
      c2PostRefresh += simpleFreecs1.getCascadeCardAt(1, i).toString() + " ";
    }
  ...
  public void testGameOver() {
    ...
    assertEquals(simpleFreecs1.isGameOver(), false);
